...
function selChk03(){
	...
	$.post('json_prod.php',{'c1':c1,'c2':c2,'c3':c3}, function(prod){
		...
		prodNum = parData.length;
		if(prodNum){
			$('#dropTxt').text('주문상품선택');
			$('#prodBtn').removeClass('btn-secondary');
			$('#prodBtn').addClass('btn-primary');
			...
			for(i=0; i<parData.length; i++){	
				...
				strHtml = "<a class='dropdown-item' href='javascript:prodAdd("+u+");'><img src='/upfile/"+img+"' style='width:50px;height:50px;'> "+txt+"</a>";
				$('#prod').append(strHtml);
			}
			...
			$('#dropTxt').text('등록상품없음');
			$('#prodBtn').removeClass('btn-primary');
			$('#prodBtn').addClass('btn-secondary');
		}
	});
}

function prodListClear(){
	$('#prod').empty();
	$('#dropTxt').text('등록상품없음');
	$('#prodBtn').removeClass('btn-primary');
	$('#prodBtn').addClass('btn-secondary');
...
$(function(){
	$('.sChk').change(function(){
		$("#loading").show();
		uid = $(this).data('uid');
		v = $(this).val();

		$.post('proc.php',{'type':'sCheck','status':v,'uid':uid}, function(){$("#loading").delay("200").fadeOut();});
	});
