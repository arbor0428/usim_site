...
if($_GET['jQueryLoad']){
	...
	//일을 선택한 경우
	if($f_day){
		$f_sTime = strtotime($fDay.$f_day);	//선택한 일(타임값)
		$f_eTime = $f_sTime + 86399;		//선택한 일(타임값)

	}else{
		$f_sTime = strtotime($fDay."01");
		$f_eTime = strtotime($fDay.$lastDay) + 86399;
	}
...
$(function(){
	$('#f_day').change(function(){
		...
		$('#pieChartWrap').load('pieChart.php?jQueryLoad=1&f_ym='+f_ym+'&f_day='+f_day);
	});
});
</script>
...
<div class="card shadow mb-4">
	...
	<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
		<h6 class="m-0 font-weight-bold text-info"><i class="fas fa-chart-pie"></i> 일별 접수 현황 (<?=number_format($dataCnt)?>건)</h6>
			<select name="f_day" id="f_day" class="form-control" style='width:120px;'>
				<option value=''><?=$f_ym?></option>
			<?
				for($i=1; $i<=$lastDay; $i++){
					if($f_day == $i)	$chk = 'selected';
					else					$chk = '';
			?>
				<option value="<?=$i?>" <?=$chk?>><?=$i?>일</option>
			<?
				}
			?>
			</select>
